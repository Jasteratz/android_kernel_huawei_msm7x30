...
static DEFINE_SEMAPHORE(mt9v114_sem);

static int mt9v114_i2c_rxdata(unsigned short saddr, unsigned char *rxdata,
	int length)
{
	int rc;
	struct i2c_msg msgs[] = {
	...
	rc = i2c_transfer(mt9v114_client->adapter, msgs, ARRAY_SIZE(msgs));
	if (rc < 0)
...
static int mt9v114_i2c_read_word(unsigned short saddr, unsigned short raddr,
	unsigned short *rdata)
{
	...
	rc = mt9v114_i2c_rxdata(saddr, buf, ARRAY_SIZE(buf));
	if (rc < 0)
		return rc;
...
static int mt9v114_i2c_txdata(unsigned short saddr, unsigned char *txdata,
	int length)
{
	int rc;
	struct i2c_msg msg[] = {
	...
	rc = i2c_transfer(mt9v114_client->adapter, msg, ARRAY_SIZE(msg));
	if (rc < 0)
		pr_err("%s: i2c_transfer failed rc=%d\n", __func__, rc);
...
static int mt9v114_i2c_write_word_once(unsigned short saddr,
	...
	unsigned short wdata)
{
	...
	buf[3] = (wdata & 0x00FF);
	rc = mt9v114_i2c_txdata(saddr, buf, ARRAY_SIZE(buf));
	return rc;
}

static int mt9v114_i2c_write_word(unsigned short saddr, unsigned short waddr,
	unsigned short wdata)
{
	int rc = -EFAULT;
	unsigned char buf[4];
	...
	/* Try 3 times, if error, return -EIO. */
	for (i = 0; i < 3; i++) {
		rc = mt9v114_i2c_txdata(saddr, buf, ARRAY_SIZE(buf));
		if (rc == 0)
			break;
...
static int mt9v114_i2c_write_byte(unsigned short saddr, unsigned short waddr,
	unsigned char cdata)
{
	int rc = -EFAULT;
	unsigned char buf[3];
...
static int mt9v114_i2c_write_table(
	struct mt9v114_i2c_reg_conf *reg_conf_tbl, int num_of_items_in_table)
{
	int i;
	int rc = -EFAULT;
	...
	for (i = 0; i < num_of_items_in_table; i++) {
		if (E_REGISTER_CMD_8BIT == reg_conf_tbl->type) {
			...
			if (rc < 0) {
				pr_err("%s: reg[%d] write failed rc=%d\n",
					__func__, i, rc);
				break;
			}
...
static int mt9v114_set_default_focus(uint8_t af_step)
{
	CDBG("%s: af_step %d\n", __func__, af_step);
	return 0;
}

static int mt9v114_set_effect(int32_t effect)
{
	struct mt9v114_i2c_reg_conf *reg_conf_tbl = NULL;
	int num_of_items_in_table = 0;
	int rc = 0;

	CDBG("%s: current_effect %d effect %d\n",
		__func__, current_effect, effect);
	switch (effect) {
	case CAMERA_EFFECT_OFF:
		reg_conf_tbl = mt9v114_effect_off_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_effect_off_reg_config);
		break;
	case CAMERA_EFFECT_MONO:
		reg_conf_tbl = mt9v114_effect_mono_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_effect_mono_reg_config);
		break;
	case CAMERA_EFFECT_NEGATIVE:
		reg_conf_tbl = mt9v114_effect_negative_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_effect_negative_reg_config);
		break;
	case CAMERA_EFFECT_SOLARIZE:
		reg_conf_tbl = mt9v114_effect_solarize_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_effect_solarize_reg_config);
		break;
	case CAMERA_EFFECT_SEPIA:
		reg_conf_tbl = mt9v114_effect_sepia_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_effect_sepia_reg_config);
		break;
	case CAMERA_EFFECT_AQUA:
		reg_conf_tbl = mt9v114_effect_aqua_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_effect_aqua_reg_config);
		break;
	default:
		return 0;
	}
	...
	if (current_effect != effect) {
		rc = mt9v114_i2c_write_table(
			reg_conf_tbl, num_of_items_in_table);
...
static int mt9v114_set_wb(int32_t wb)
{
	struct mt9v114_i2c_reg_conf *reg_conf_tbl = NULL;
	int num_of_items_in_table = 0;
	int rc = 0;
	...
	case CAMERA_WB_AUTO:
		...
		num_of_items_in_table = ARRAY_SIZE(mt9v114_wb_auto_reg_config);
		break;
	case CAMERA_WB_INCANDESCENT:
		reg_conf_tbl = mt9v114_wb_incandescent_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_wb_incandescent_reg_config);
		break;
	case CAMERA_WB_CUSTOM:
	case CAMERA_WB_FLUORESCENT:
		reg_conf_tbl = mt9v114_wb_fluorescent_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_wb_fluorescent_reg_config);
		break;
	case CAMERA_WB_DAYLIGHT:
		reg_conf_tbl = mt9v114_wb_daylight_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_wb_daylight_reg_config);
		break;
	case CAMERA_WB_CLOUDY_DAYLIGHT:
		reg_conf_tbl = mt9v114_wb_cloudy_reg_config;
		num_of_items_in_table =
			ARRAY_SIZE(mt9v114_wb_cloudy_reg_config);
		break;
	default:
		return 0;
	}
...
static int mt9v114_set_fps(struct fps_cfg *fps)
{
	CDBG("%s\n", __func__);
	return 0;
}

static int mt9v114_write_exp_gain(uint16_t gain, uint32_t line)
{
	CDBG("%s\n", __func__);
	return 0;
}

static int mt9v114_set_pict_exp_gain(uint16_t gain, uint32_t line)
{
	CDBG("%s\n", __func__);
	return 0;
}

static int mt9v114_setting(enum mt9v114_reg_update_t rupdate,
	enum mt9v114_setting_t rt)
{
	...
	case REG_INIT:
		rc = mt9v114_i2c_write_table(mt9v114_init_reg_config,
			ARRAY_SIZE(mt9v114_init_reg_config));
		if (rc < 0)
			pr_err("%s: mt9v114_i2c_write_table failed rc=%d\n",
				__func__, rc);
...
static int mt9v114_video_config(int mode, int res)
{
	int rc;
	...
	case QTR_SIZE:
		...
		if (rc < 0) {
			pr_err("%s: mt9v114_setting failed rc=%d\n",
				__func__, rc);
			return rc;
		}
...
static int mt9v114_snapshot_config(int mode)
{
	int rc = 0;
...
static int mt9v114_power_down(void)
{
	return 0;
}

static int mt9v114_move_focus(int direction, int32_t num_steps)
{
	return 0;
}
...
static int mt9v114_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	/* Reset the sensor via I2C command. */
	rc = mt9v114_i2c_write_word_once(mt9v114_client->addr,
	...
	if (chipid != MT9V114_MODEL_ID) {
		pr_err("%s: model id does not match 0x%x!=0x%x\n",
			__func__, chipid, MT9V114_MODEL_ID);
		rc = -ENODEV;
		goto init_probe_fail;
	}
...
static int mt9v114_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	int rc;
	current_effect = CAMERA_EFFECT_OFF;

	mt9v114_ctrl = kzalloc(sizeof(*mt9v114_ctrl), GFP_KERNEL);
	if (!mt9v114_ctrl) {
...
static int mt9v114_set_sensor_mode(int mode, int res)
{
	int rc = 0;
...
static int mt9v114_sensor_config(void __user *argp)
{
	...
	case CFG_SET_EXP_GAIN:
		rc = mt9v114_write_exp_gain(cdata.cfg.exp_gain.gain,
			cdata.cfg.exp_gain.line);
		break;
	case CFG_SET_PICT_EXP_GAIN:
		rc = mt9v114_set_pict_exp_gain(cdata.cfg.exp_gain.gain,
			cdata.cfg.exp_gain.line);
		break;
	case CFG_SET_MODE:
		rc = mt9v114_set_sensor_mode(cdata.mode, cdata.rs);
		break;
	case CFG_PWR_DOWN:
		rc = mt9v114_power_down();
		break;
	case CFG_MOVE_FOCUS:
		rc = mt9v114_move_focus(cdata.cfg.focus.dir,
			cdata.cfg.focus.steps);
		break;
	case CFG_SET_DEFAULT_FOCUS:
		rc = mt9v114_set_default_focus(cdata.cfg.focus.steps);
		break;
	case CFG_SET_EFFECT:
		rc = mt9v114_set_effect(cdata.cfg.effect);
		break;
	case CFG_SET_WB:
		rc = mt9v114_set_wb(cdata.cfg.effect);
		break;
	default:
		rc = -EFAULT;
		break;
	...
	struct msm_sensor_ctrl *s)
{
	...
	if (rc < 0) {
		...
		pr_err("%s: mt9v114_probe_init_sensor failed rc=%d\n",
			__func__, rc);
		return rc;
	}
